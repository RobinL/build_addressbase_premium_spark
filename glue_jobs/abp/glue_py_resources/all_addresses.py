...
def create_views_on_parquet(spark):
    ...
    for k,v in table_names.items():
        try:
            df = spark.read.parquet("s3a://alpha-everyone/deleteathenaout/abpparquet2/{}".format(v["tablename"]))
...
def geographic_addresses(spark):
    ...
    (case when o.organisation is not null then o.organisation || ' ' else '' end
...
def create_all_addresses(spark):
    ...
    all_addresses.write.parquet("s3a://alpha-everyone/deleteathenaout/abpparquet2/all_addresses", mode="overwrite")
